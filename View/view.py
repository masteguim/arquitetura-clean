...
class ContaView(Interfaceconta):
    def __init__(self, root):
        ...
        tk.Label(self.root, text="CPF").grid(row=1, column=0, padx=10, pady=10)
        self.ent_cpf = tk.Entry(self.root, width=30)
        self.ent_cpf.grid(row=1, column=1)

        tk.Label(self.root, text="Saldo Inicial R$:").grid(row=2, column=0, padx=10, pady=10)
        self.ent_saldo_inicial = tk.Entry(self.root, width=30)
        self.ent_saldo_inicial.grid(row=2, column=1)
        ...
        tk.Label(self.root, text="Valor").grid(row=4, column=0, padx=10, pady=10)
        self.ent_valor = tk.Entry(self.root, width=30)
        self.ent_valor.grid(row=4, column=1)

        tk.Button(self.root, text="Depositar", command=self.ao_clicar_depositar).grid(row=5, column=0, pady=10)
        tk.Button(self.root, text="Sacar", command=self.ao_clicar_sacar).grid(row=5, column=1, pady=10)
        tk.Button(self.root, text="Histórico", command=self.ao_clicar_historico).grid(row=6, column=0, columnspan=2, pady=10)
        tk.Button(self.root, text="Rendimento", command=self.ao_clicar_rendimento).grid(row=7, column=0, columnspan=2, pady=10)
    ...
    def ao_clicar_rendimento(self):
        if self.controller:
            self.controller.aplicar_rendimento()
